...
function trimString(original, side) {
  if (side === "start") {
    return original.trimStart(); // Видаляє пробіли на початку рядка
  } else if (side === "end") {
    return original.trimEnd(); // Видаляє пробіли в кінці рядка
  } else if (side === "both") {
    return original.trim(); // Видаляє пробіли з обох сторін
  } else {
    return "Error: side should be either 'start', 'end' or 'both'"; // Помилка, якщо side не є правильним значенням
  }
...
function convertCase(original, caseType) {
  if (caseType === "upper") {
    return original.toUpperCase(); // Перетворює рядок у верхній регістр
  } else if (caseType === "lower") {
    return original.toLowerCase(); // Перетворює рядок у нижній регістр
  } else {
    return "Error: caseType should be either 'upper' or 'lower'"; // Помилка, якщо caseType не є правильним значенням
  }
...
function substringString(string, startIndex, endIndex) {
  const before = string.substring(0, startIndex);
  const after = string.substring(endIndex);
